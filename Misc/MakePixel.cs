...
namespace BFL
{
    public class MakePixel : MonoBehaviour
    {
        public Camera cam;
        ...
        void Update()
        {
            if (Input.GetKeyDown(KeyCode.Space) && !makePixelActive)
            {
                StartCoroutine(nameof(MakePixelCoroutine));
            }
        }

        void Paint(Vector3 rayPos)
        {
            RaycastHit hit;
        ...
        IEnumerator MakePixelCoroutine()
        {
