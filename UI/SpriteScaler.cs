namespace BFL.UI
{
    ...
    public class SpriteScaler : MonoBehaviour
    {
        ...
        [HideInInspector] [SerializeField] private float baseRatioOld = 0.0f;

        [Range(0, 1)] [SerializeField] private float Height_Width = 1;
        [HideInInspector] [SerializeField] private float Height_WidthOld = 1;

        [SerializeField] private float additionalScale = 1;
        [HideInInspector] [SerializeField] private float additionalScaleOld = 1;

        [SerializeField] private bool useLerpScale = false;
        private LerpScale lerpScale_0_5;

        #region temp vars

        [SerializeField] private float currRatio;
        private int width = 0;
        private int height = 0;
        private float sc = 1f;
        private bool debug = false;

        [SerializeField] private float lerpRatio = 1;
        ...
        void SetScale()
        {
            ...
            if (!useLerpScale)
            {
                if (width != Screen.width || height != Screen.height || baseRatio != baseRatioOld ||
                    Height_Width != Height_WidthOld || additionalScaleOld != additionalScale)
                {
                    width = Screen.width;
            ...
            else
            {
                if (width != Screen.width || height != Screen.height || baseRatio != baseRatioOld ||
                    Height_Width != Height_WidthOld || additionalScaleOld != additionalScale ||
                    lerpRatio != lerpRatioOld || lerpScale != lerpScaleOld)
                {
                    width = Screen.width;
                    height = Screen.height;
                    currRatio = width / (float)height;
            ...
            if (debug)
                Debug.Log("width: " + width + " ; height: " + height + " ;baseW/baseH: " + baseRatio + "currW/currH: " +
                          currRatio + " ;scale: " + sc);
        }
        ...
        void BaseScale()
        {
            ...
            sc = (sc >= 1)
                ? Mathf.Lerp(1, sc, Height_Width) * additionalScale
                : Mathf.Lerp(sc, 1, Height_Width) * additionalScale;
